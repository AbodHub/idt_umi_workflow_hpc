...
pathkeys = {"refgenome", "captureregions", "captureregionsil", "samplefile", "baseoutdir"}  # config elements that are filepaths
...
rule fgbio_annotate_umis:
    ...
    params:
        ...
        fgbio = config["cappseq_umi_workflow"]["fgbio"]
    log:
        outdir + os.sep + "logs" + os.sep + "{samplename}.annotateumis.log"
    envmodules:
        "java"
    shell:
        "java -Xmx2048m -jar {params.fgbio} AnnotateBamWithUmis --input {input.bam} --fastq {input.r1} --fastq {input.r2} --sorted true --read-structure {params.umiloc} --output {output.bam} &> {log}"
...
rule fgbio_group_umis:
    ...
    params:
        ...
        sam_threads = config["cappseq_umi_workflow"]["samtools_sort_threads"],
        fgbio = config["cappseq_umi_workflow"]["fgbio"]
    threads:
        config["cappseq_umi_workflow"]["fgb_groupumi"]
    resources:
        mem_mb = "4G"
    envmodules:
        "samtools",
    ...
    shell:
        "samtools sort -m {resources.mem_mb} -@ {params.sam_threads} -n {input.bam} | java -Xmx2048m -jar {params.fgbio} SetMateInformation --ref {input.refgenome} 2> {log} | java -Xmx2048m -jar {params.fgbio} GroupReadsByUmi --edits {params.maxedits} --family-size-histogram {output.txt} --strategy paired > {output.bam} 2>> {log}"
...
rule fgbio_duplex_consensus:
    ...
    shell:
        "java -Xmx2048m -jar {params.fgbio} CallDuplexConsensusReads --input {input.bam} --output {output.bam} --threads {threads} --read-group-id {params.sampleid} --min-reads {params.minreads} &> {log}"
...
rule bwa_realign_bam:
    ...
    shell:
        ...
        "java -jar $EBROOTPICARD/picard.jar SortSam -SO queryname -I /dev/stdin -O /dev/stdout | java -Xmx2048m -jar {params.fgbio} SetMateInformation --ref {input.refgenome} --output {output.bam} &> {log}"
...
rule picard_annotate_bam:
    ...
    shell:
        "java -jar $EBROOTPICARD/picard.jar SortSam -I {input.unaligned_bam} -O /dev/stdout -SO queryname --REFERENCE_SEQUENCE {input.refgenome} | java -jar $EBROOTPICARD/picard.jar MergeBamAlignment --ALIGNED_BAM {input.aligned_bam} --UNMAPPED_BAM /dev/stdin --REFERENCE_SEQUENCE {input.refgenome} -O {output.bam} &> {log} && "
...
rule picard_validate_sam:
    ...
    shell:
        "java -jar $EBROOTPICARD/picard.jar ValidateSamFile -I {input.bam} -R {input.refgenome} > {output.txt} 2> {log}"
...
rule qc_picard_hsmetrics:
    ...
    shell:
        "java -jar $EBROOTPICARD/picard.jar CollectHsMetrics -R {input.refgenome} -TI {params.capture_reg_il} -BI {params.capture_reg_il} -I {input.bam} -O {output.hsmet} --PER_TARGET_COVERAGE {output.tarcov} --MAX_RECORDS_IN_RAM {params.max_ram_records} --COVERAGE_CAP {params.cov_cap_sens} 2> {log}"
...
rule qc_picard_oxog:
    ...
    shell:
        "java -jar $EBROOTPICARD/picard.jar CollectOxoGMetrics -I {input.bam} -R {input.refgenome} -O {output.txt} 2> {log}"
...
rule qc_picard_insertsize:
    ...
    shell:
        "java -jar $EBROOTPICARD/picard.jar CollectInsertSizeMetrics -I {input.bam} -O {output.txt} -H {output.pdf} 2> {log}"
...
rule qc_fgbio_errorrate:
    ...
    params:
        ...
        capture_reg_il = config["cappseq_umi_workflow"]["captureregionsil"],
        #dbsnpvcf = config["cappseq_umi_workflow"]["dbsnpvcf"], # got rid of -v {params.dbsnpvcf} for now.
        fgbio = config["cappseq_umi_workflow"]["fgbio"]
    envmodules:
        "java"    
    log:
    ...
    shell:
        "java -Xmx2048m -jar {params.fgbio} ErrorRateByReadPosition -i {input.bam} -r {input.refgenome} -l {params.capture_reg_il} --collapse -o {params.outprefix} 2> {log}"
